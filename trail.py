...
from basicsr.models.archs.WaveNet_arch import WaveNet_T,WaveNet_B
from waternet.trail import transform_image,transform_array_to_image,load_model

def run():
    device=torch.device(device="cuda" if torch.cuda.is_available() else "cpu")
    ckpt="/home/muahmmad/projects/Image_enhancement/WaveNet/checkpoints/5k/WaveNet_B_5k.pth"
    model=load_model(ckpt_dir=ckpt,
                     model_type=WaveNet_B,
                     device=device)
    image = cv2.imread(
